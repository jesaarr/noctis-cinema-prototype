...
export default function Sidebar({ activeTab, setActiveTab, setSelectedMovieId, onViewProfile }: SidebarProps) {
  const { history, watchLater, likedVideos, username } = useNoctisStorage();
    ...
    <aside className="w-64 min-h-screen bg-[#070709] border-r border-[#121214] p-6 flex flex-col justify-between select-none">
      <div className="space-y-8">
        ...
        <div className="bg-[#0b0b0e] border border-[#16161a] p-5 rounded-2xl relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-noctis-gold/20 to-transparent" />
          
          <div className="space-y-4">
            <div className="space-y-1">
              <p className="text-[10px] font-mono tracking-widest text-noctis-muted uppercase">SYS_CURATOR</p>
            ...
            <button 
              ...
              className="w-full py-2 px-3 bg-transparent border border-white/[0.03] hover:border-noctis-gold text-noctis-muted hover:text-noctis-gold text-[9px] font-mono uppercase tracking-widest rounded-lg transition-all duration-300 ease-out"
            >
              VIEW PROFILE
            </button>
          </div>
        ...
        <nav className="space-y-7">
          ...
          <div className="space-y-1.5">
            <p className="text-[9px] text-noctis-muted font-mono tracking-[0.3em] uppercase font-medium px-3 mb-3">
              DISCOVER
            </p>
            ...
            <button 
              ...
              className={`w-full text-left py-2.5 px-3 rounded-xl text-xs tracking-wider transition-all duration-300 relative group flex items-center justify-between ${
                activeTab === 'explore' 
                  ? 'bg-gradient-to-r from-noctis-gold/5 to-transparent text-noctis-gold font-medium' 
                  : 'text-noctis-muted hover:bg-white/[0.01] hover:text-gray-200'
              }`}
            >
              <div className="flex items-center gap-2">
                {activeTab === 'explore' && (
                  <span className="w-1.5 h-1.5 bg-noctis-gold rounded-full shadow-[0_0_8px_rgba(212,175,55,0.35)]" />
                )}
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <span className="text-[9px] font-mono text-noctis-muted bg-noctis-card px-2 py-0.5 rounded border border-white/[0.05] group-hover:text-gray-300">
                {historyCount}
              </span>
            </button>
      ...
      <div className="border-t border-[#121214] pt-6 flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 bg-noctis-gold/80 rounded-full animate-pulse" />
          <p className="text-[8px] text-noctis-muted font-mono tracking-[0.2em] uppercase font-light">
            SECURE TRANSMISSION
          </p>
        </div>
        <p className="text-[7px] text-noctis-muted font-mono tracking-wider">
          NOCTIS ENGINE v2.5
        </p>
      </div>
    </aside>
